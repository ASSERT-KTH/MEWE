...
import re
DRAW_CURVE_OVER = True
def filteroutliers(data):
    q1o = np.quantile(data, 0.2)
...
if __name__ == "__main__":
    latexify(fig_width=3.5, fig_height=3.5, font_size=10, tick_size=10)
    ...
    MAX =100000
    times = data[name]['pureRandom']['times'][0][:MAX]
    paths = data[name]['pureRandom']['times'][1][:MAX]
    ...
    if PLOT_TPE == 'all':
        ...
        ax1.plot(list(range(len(times))), times)
    elif PLOT_TPE == 'cumulative':
        ...
        for k, ts  in groups.items():
            if len(ts) > 10:
                ...
                #x = np.sort(ts)
                # scaling 
                x = [t/1000 for t in ts] # microsecondss
                x = filteroutliers(x)
                print(len(x), len(ts))
                no, binso, patcho = ax1.hist(x, bins='auto',  alpha=0.1, density=True)
                no, binso, patcho = ax1.hist(x, bins='auto', color='C1', alpha=0.1, density=True, histtype='step',linewidth=1.5,)

                # Draw curve over
                if False:
                    intervals = 1000

                    gaussian_kde_zi = stats.gaussian_kde(x)
                    gaussian_kde_zi.covariance_factor = lambda : 0.3
                    gaussian_kde_zi._compute_covariance()
                    y=np.linspace(min(x), max(x), intervals)
                    ax1.plot(y, gaussian_kde_zi(y),'--' ,  linewidth=1.5, color='C1', label='kde', alpha=0.1)
