...
def process_bitcode(t):
    ...
    if not os.path.exists(f"{bitcode_folder}/bitcodes/variants") or len(os.listdir(f"{bitcode_folder}/bitcodes/variants")) == 0:
        print(f"No variants for {bitcode_folder}")
        return result
    # generate original wasm
    if not os.path.exists(original_bitcode):
        print(f"No original for {bitcode_folder}")
