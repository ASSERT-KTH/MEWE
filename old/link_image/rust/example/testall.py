...
BOT_TOKEN=os.environ.get("BOT_API_TOKEN", None)
BOT_CHANNEL=os.environ.get("BOT_API_CHANNEL", None)

myclient = pymongo.MongoClient(f"mongodb://{MONGO_USER}:{MONGO_PASS}@localhost:27017/")
mydb = myclient["fastly4edge"]

image = open("kth.png", "rb").read()
...
def deploy(mainContent, bitcodes_folder="original", save_as=None, save_sec_as="sec.txt"):
    ...
    # check file size
    wasmSize = len(open("example_img.wasm", 'rb').read())
    print(wasmSize)
    try:
        metadata = process_wasm_stats("example_img.wasm")
    except:
        metadata = { }
    if save_as:
        shutil.copy("example_img.wasm", f"out/{save_as}")
    # execute wasmbench security analysis tool

    
    return wasmSize, metadata, out

def execute_to_time(service_name, times=1000, meta={}):

    result = []
    print("Getting execution time distribution")
    for t in range(times):
        response = requests.get(service_name, data=image)
        ...
        if 'xtime' in response.headers:
            ...
            if BOT_TOKEN != None and t % 100 == 0:
                requests.get(
                    f"https://api.telegram.org/bot{BOT_TOKEN}/sendMessage",
                    data=dict(
                        chat_id=BOT_CHANNEL,
                        text=f"Response from success in time {response.headers['xtime']} iteration {t}",
                        parse_mode="HTML"
                    )
                )
                a=open("out/r.png", 'wb')
...
def pex(p):
    ...
    try:
        for it in range(times):
            ...
            # sending notification to telegram boot
            if BOT_TOKEN != None  and it %20 == 0:
                requests.get(
                    f"https://api.telegram.org/bot{BOT_TOKEN}/sendMessage",
                    data=dict(
                        chat_id=BOT_CHANNEL,
                        text=f"Response from {popl}-{pcNumber} success",
                        parse_mode="HTML"
                    )
                )
            ...
            # curl --data parse_mode=HTML --data chat_id="665043934" --data "text=$1" "https://api.telegram.org/bot$BOT_API_TOKEN/sendMessage"
...
def execute_paths(service_name, t=100, meta={}):
...
def test_with_template(case, template_name, extract_paths=True, extract_times=True, bitcodes_folder="multivariant", t=1):
    mname, _   = case

    MAIN_TEMPLATE = open(f"{template_name}", 'r').read()
    ...
    times = execute_to_time("https://totally-devoted-krill.edgecompute.app", meta=record, times=1000) if extract_times else { }
...
def test_no_diversification(name, bitcode, template):
    ...
    MAIN_TEMPLATE = open(f"{template}", 'r').read()
    ...
    print("rPath size", rPathSize)
    withrPathTimes = execute_to_time("https://totally-devoted-krill.edgecompute.app", times=1000)
...
def test_all():
    ...
    cases = [
        (
            "fliph",[
            ("all.bc", "templates/cases/fliph/fliph_main.rs"), # original
            ("allinone.multivariant.i.bc", "templates/cases/fliph/fliph_paths.rs"), # instrumented for path
            ("allinone.multivariant.bc", "templates/cases/fliph/fliph.rs")] # multivariant
        ),(
            "flipv",[
            ("all.bc", "templates/cases/flipv/flipv_main.rs"), # original
            ("allinone.multivariant.i.bc", "templates/cases/flipv/flipv_paths.rs"), # instrumented for path
            ("allinone.multivariant.bc", "templates/cases/flipv/flipv.rs")] # multivariant
        ),(
            "blur",[
            ("all.bc", "templates/cases/blur/blur_main.rs"), # original
            ("allinone.multivariant.i.bc", "templates/cases/blur/blur_paths.rs"), # instrumented for path
            ("allinone.multivariant.bc", "templates/cases/blur/blur.rs")] # multivariant
        ),(
            "crop",[
            ("all.bc", "templates/cases/crop/crop_main.rs"), # original
            ("allinone.multivariant.i.bc", "templates/cases/crop/crop_paths.rs"), # instrumented for path
            ("allinone.multivariant.bc", "templates/cases/crop/crop.rs")] # multivariant
        ),(
            "gray",[
            ("all.bc", "templates/cases/gray/gray_main.rs"), # original
            ("allinone.multivariant.i.bc", "templates/cases/gray/gray_paths.rs"), # instrumented for path
            ("allinone.multivariant.bc", "templates/cases/gray/gray.rs")] # multivariant
        ),(
            "invert",[
            ("all.bc", "templates/cases/invert/invert_main.rs"), # original
            ("allinone.multivariant.i.bc", "templates/cases/invert/invert_paths.rs"), # instrumented for path
            ("allinone.multivariant.bc", "templates/cases/invert/invert.rs")] # multivariant
        ),(
            "rotate90",[
            ("all.bc", "templates/cases/rotate90/rotate_main.rs"), # original
            ("allinone.multivariant.i.bc", "templates/cases/rotate90/rotate_paths.rs"), # instrumented for path
            ("allinone.multivariant.bc", "templates/cases/rotate90/rotate.rs")] # multivariant
        ),(
            "unsharp",[
            ("all.bc", "templates/cases/unsharp/unsharp_main.rs"), # original
            ("allinone.multivariant.i.bc", "templates/cases/unsharp/unsharp_paths.rs"), # instrumented for path
            ("allinone.multivariant.bc", "templates/cases/unsharp/unsharp.rs")] # multivariant
        ),(
            "fractal",[
            ("all.bc", "templates/cases/fractal/fractal_main.rs"), # original
            ("allinone.multivariant.i.bc", "templates/cases/fractal/fractal_paths.rs"), # instrumented for path
            ("allinone.multivariant.bc", "templates/cases/fractal/fractal.rs")] # multivariant
        ),
    ]
...
if __name__ == "__main__":
    if BOT_TOKEN != None:
        print(f"Sending starting message {BOT_TOKEN} {BOT_CHANNEL}")
        requests.get(
            f"https://api.telegram.org/bot{BOT_TOKEN}/sendPhoto",
            data=dict(
                chat_id=BOT_CHANNEL,
                text=f"Starting with image_rs",
                parse_mode="HTML",
                
            ),
                files = {
                    'photo': open("kth.png", 'rb')
                }
            )
