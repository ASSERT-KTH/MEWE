...
class EdgeQuery:
	...
	def set_node(self, fname, tpe):
		with self.driver.session() as session:
			subpgrah = session.read_transaction(self._create_and_return_greeting, tpe)
 #session.read_transaction(self._create_and_return_greeting, fname, tpe)
	...
	def _create_and_return_greeting(tx, tpe):
		...
		c  = tx.run(f"MATCH p=(start)-[:{tpe}]->(k) unwind p as paths return paths")
		return c
...
if __name__ == "__main__":
	...
	for fname, tpe in zip(sys.argv[2::2], sys.argv[3::2]):
		...
		#print(f"## {tp}")
			...
			# print(visited)
			dfs(f'{fname}_entry', f'{fname}_end', visited, D, lambda: globals().update(COUNT=COUNT + 1))
