...
if [[ "$OSTYPE" == "linux-gnu"* ]]; then
    sudo apt install libproj-dev proj-bin libgeos-dev tshark proj-bin docker.io -y
fi

sudo docker rm mongodb --force
sudo docker rm some-rabbit --force

sudo docker run -d --restart always   -v mongodata:/data/db  -p 27017:27017 -e MONGO_INITDB_ROOT_USERNAME=$MONGO_USER -e MONGO_INITDB_ROOT_PASSWORD=$MONGO_PASS --name mongodb -d mongo
